/* ============================================================
   ...
   Cursor + Navbar from working version
   Form saves to backend only — NO WhatsApp redirect
   ============================================================ */
...
/* ════════════════════════════════════════════════════
   QUOTATION FORM
   Handles both onsubmit="submitQuote(event)" (old HTML)
...
// ── Global submitQuote function (called from onsubmit in HTML) ──
window.submitQuote = async function(e) {
  if (e) e.preventDefault();

  // Try both old and new form IDs
  ...
  // Get values — try multiple possible field IDs/names
  ...
  try {
    const res  = await fetch(`${BACKEND}/api/quotations`, {
      ...
      body: JSON.stringify({ name, phone, email, city: city || 'Not specified', service: service || 'Not specified', floors, building_type, message }),
    });
    ...
    errMsg = 'Cannot reach server. Make sure backend is running (npm start).';
  ...
  if (success) {
    // Hide form, show success — stays on page
    ...
    if (successEl) {
      ...
      if (p)  p.textContent  = 'Thank you! Our team will review your details and contact you within 2 hours via call or WhatsApp.';
    }
  } else {
...
// ── Also attach via addEventListener for new HTML ──
...
function showFormAlert(form, msg, type) {
  ...
  el.style.cssText = `
    ...
    padding:0.6rem 1rem;
    background:${type === 'error' ? 'rgba(255,77,77,0.1)' : 'rgba(37,211,102,0.1)'};
    border:1px solid ${type === 'error' ? 'rgba(255,77,77,0.3)' : 'rgba(37,211,102,0.3)'};
    border-radius:6px;
  `;
  el.textContent = msg;
  const btn = form.querySelector('.form-submit, button[type="submit"], .btn-submit');
...
console.log('%c✅ All systems operational 🏢⚡', 'color:#00c8d4;font-family:monospace');